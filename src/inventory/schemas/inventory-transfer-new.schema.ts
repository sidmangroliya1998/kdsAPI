...
export type InventoryTransferNewDocument = InventoryTransferNew & Document;
...
const TransferInventoryItemSchema = SchemaFactory.createForClass(TransferInventoryItem);


@Schema({ timestamps: true })
export class InventoryTransferNew {
  @Prop({
    type: MongooseSchema.Types.ObjectId,
    ref: 'Supplier',
    index: true,
  ...
  @Prop({ type: [TransferInventoryItemSchema], required: true })
  items: TransferInventoryItem[];
...
export const InventoryTransferNewSchema =
  SchemaFactory.createForClass(InventoryTransferNew);

InventoryTransferNewSchema.plugin(paginate);
